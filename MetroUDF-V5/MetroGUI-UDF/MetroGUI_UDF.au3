...
Func _Metro_SetGUIOption($mGUI, $AllowDragMove = False, $AllowResize = False, $Win_MinWidth = "", $Win_MinHeight = "")
	...
	If ($iGui_Count == "") Then
		...
		ReDim $iGUI_LIST[$iGui_Count + 1][12]
	EndIf
...
Func _Metro_GUIDelete($GUI)
	...
	For $i_HR = 0 To UBound($iGUI_LIST) - 1 Step +1
		If $iGUI_LIST[$i_HR][0] <> $GUI Then
			ReDim $CLEANED_GUI_LIST[UBound($CLEANED_GUI_LIST) + 1][12]
...
Func _Metro_AddControlButtons($CloseBtn = True, $MaximizeBtn = True, $MinimizeBtn = True, $FullScreenBtn = False, $MenuBtn = False, $GUI_BG_Color = $GUIThemeColor, $GUI_Font_Color = $FontThemeColor)
	...
	Return _iCreateControlButtons($ButtonsToCreate_Array, $GUI_BG_Color, $GUI_Font_Color, False)
...
Func _iCreateControlButtons($ButtonsToCreate_Array, $GUI_BG_Color = $GUIThemeColor, $GUI_Font_Color = "0xFFFFFF", $CloseButtonOnStyle = False)
	;HighDPI Support
	Local $cbDPI = _HighDPICheck()
			...
			If Not $iHoverReg[$i][2] Then
				If Not $NoAnimation Then
					For $i2 = 6 To 11 Step +1
						...
						Sleep(1)
					Next
					_WinAPI_DeleteObject(GUICtrlSendMsg($Toggle, 0x0172, 0, $iHoverReg[$i][14]))
				Else
					_WinAPI_DeleteObject(GUICtrlSendMsg($Toggle, 0x0172, 0, $iHoverReg[$i][14]))
				EndIf
				$iHoverReg[$i][2] = True
...
; Syntax ........: _Metro_CheckboxUnCheck($Checkbox, $NoHoverEffect = False)
; Parameters ....: $Checkbox            - Handle to the Checkbox.
;							  $NoHoverEffect	  - Default = False, True = Prevents the hover effect from appearing/freezing / Should be used during startup or anytime when the function is called by clicking manually on the checkbox.
; ===============================================================================================================================
Func _Metro_CheckboxUnCheck($Checkbox, $NoHoverEffect = False)
	For $i = 0 To (UBound($iHoverReg) - 1) Step +1
		If $iHoverReg[$i][0] = $Checkbox Then
			$iHoverReg[$i][2] = False
			$iHoverReg[$i][1] = True
			If $NoHoverEffect Then
				_WinAPI_DeleteObject(GUICtrlSendMsg($Checkbox, 0x0172, 0, $iHoverReg[$i][5]))
			Else
